import type { GetPageResult, Page } from "./getPages";
...
export const pageResolver = async (pageResult: GetPageResult) => {
  if (!pageResult.page || pageResult.error) return pageResult;
  const pageBlocks = pageResult.page.blocks || [];
  const page: Page = pageResult.page;
  ...
  for (let i = 0; i < pageBlocks.length; i++) {
    ...
    if (ob?.item?.__typename === "block_posts") {
      ...
      if (topic) {
        ...
        const active = pageResult.number;
  ...
  page.blocks = blocks;
  const result: GetPageResult = { ...pageResult, page: page };
  return result;
};
