...
export default async function getPosts(opts?: getPostsOpts): Promise<GetPostsResult | undefined> {
  ...
  const result: GetPostsResult = { posts, stale, error };
  return result;
}
...
export async function getPost(opts?: getPostsOpts): Promise<GetPostResult | undefined> {
  const res = await getPosts(opts);
  if (res?.posts.length !== 1) return undefined;
  const { error, stale } = res;
  const post = res.posts[0];
  const result: GetPostResult = { post, error, stale };
  return result;
}
